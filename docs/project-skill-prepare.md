...
Focused wrapper: 12 passing tests on local Windows. Wrapper plus scoping: 56 passing tests. The Windows-specific 8.3 regression is skipped on Ubuntu. Full verify and stored routing evaluators are required on delivery; stored historical partial/unknown denominators remain unchanged. Do not reuse these stored metrics as new live results. The [evidence summary](decision18-prepare-evidence.json) separates placement, visibility, body reads and host observations. Codex ran version 0.154.0 with observed model `gpt-5.6-sol`, effort `high`, and read-only sandbox; the positive made five read/inspection commands and one confirmed Skill body read, the negative made zero commands.

Initial hosted Windows CI run `34733699574` exposed a wrapper root-comparison defect: Node's non-native realpath retained an 8.3 path spelling while Git returned the long spelling. A new Windows regression reproduced `INVALID_PROJECT_ROOT` locally before the fix. Comparing both paths with `fs.realpathSync.native` fixes the mismatch without relaxing root or symlink checks. Production remains 61 nonblank lines. Existing live evidence predates this one-line root-spelling correction; it was not rerun or relabeled as a new live session.
